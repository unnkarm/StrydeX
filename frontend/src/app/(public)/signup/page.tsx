...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(role === "athlete" ? "/onboarding" : "/dashboard");
    } catch (err) {
      ...
      <div className="hidden flex-col justify-between bg-surface p-10 md:flex">
        ...
        <div>
          <p className="display-heading text-3xl leading-tight text-foreground">
            &ldquo;Track it. Verify it.
            <br />Get discovered.&rdquo;
          </p>
          <div className="lane-rule my-6" />
          <p className="max-w-xs text-sm text-muted">
            Join athletes building a coach-verified, scout-searchable record of their progress.
          </p>
        </div>
        <div />
      </div>
