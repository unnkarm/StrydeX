...
def _predict_generic_performance(input_data: dict) -> float:
    """Use the generic model for sports not represented by the CrossFit data."""
        ...
        [
            {
                ...
                "previous_competition_performance": numeric_values[
    ...
    try:
        ...
        predicted_performance = float(
            analytics.performance_model.predict(processed_input)[0]
        )
    except Exception as exc:
        raise HTTPException(
            status_code=500,
    ...
    return round(max(0.0, min(100.0, predicted_performance)), 2)
